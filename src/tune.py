...
def main(args):
    ...
    with open(os.path.join(args.model_dir, "metrics_best.json"), "w") as f:
        json.dump({"best_params": gs.best_params_, "cv_best_score": gs.best_score_, "holdout": report}, f, indent=2)

    import joblib
    joblib.dump({'X_test': Xte.tolist(), 'y_test': yte.tolist()}, os.path.join(args.model_dir, 'holdout.joblib'))
